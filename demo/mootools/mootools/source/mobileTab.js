...
Tabs.Effects = {
    ...
    scroll: function(fromEls, toEls, index, direction,isCircle) {
		...
		if(isCircle && activeIndex === 0 && index === len - 1){
			isBackward = true;
		}else if(isCircle && activeIndex === len-1 && index === 0){
			isBackward = false;
		}
        ...
        this.anim=new Fx.Tween(content,{duration:this.options.duration,
            onComplete:function(){
                ...
                this.anim = undefined;
				if(options.autoplay && !this.autoTimer) this.startAutoplay();
...
var Switchable=new Class({
    ...
    switchView: function(fromEls, toEls, index, direction) {
		var options= this.options, effect = options.effect,
		circular=options.circular, isCircle=options.isCircle,
		fn = typeOf(effect)=='function' ? effect : Effects[effect];
        if(circular)direction=options.direction;
        if(fn)fn.call(this, fromEls, toEls,index, direction,isCircle);
    }
});
...
var MobileSlide = new Class({
	...
	triggersEvent:function(){
		this.parent().container.addEvent('swipe',function(e){
		this[e.direction ==='left' ?'next':'prev']();
		}.bind(this));
		return this;
	},
	prev: function() {
		if(!this.options.isCircle && !this.activeIndex)return this;
		this.stopAutoplay();
		this.parent();
	},
	next: function() {
		if(!this.options.isCircle && this.activeIndex===this.length-1)return this;
		this.stopAutoplay();
		this.parent();
	},
	switchView:function(fromEls, toEls, index, direction){
		this.parent(fromEls, toEls, index, direction);
		var options = this.options;
		options.isSync && this.request(index,options.syncOpt);
	},
	request:function(index,options){
		var sync, trigger = this.triggers[index],
			update = this.panels[index],
			url = options.url ? options.url : trigger.get('data-sync-url');

		if(trigger.retrieve('cache:uid'))return this;
		if(sync && sync.running)this.cancel();
		return sync = this.sync = new Request.HTML(
			Object.append(options,{
				url:url,
				update:update,
				onSuccess:function(rs){trigger.store('cache:uid',true);}
			})).send();
	}
});
...
var ScrollRequest = new Class({
    ...
    initialize:function(main,options){
		this.main = main;
        options = this.setOptions(options).fireEvent('load').options;
		this.container = $(options.container) || window;
        this.initEvent().step=1;
    },
    initEvent:function(){
        var timer, self = this, win = window;
        win.addEvent('domready',this.progress.bind(this));
        this.container.addEvents({'scroll':loader,'resize':loader});
        ...
        function loader(){
			...
			timer = function(){
				...
				if(self.isComplete)
			    self.container.removeEvent('scroll',loader).removeEvent('resize',loader);
				timer = null;
          }.delay(200);
        };
		return this;
    ...
    progress :function(){
        this.filterItems() && this.request();
	},
    filterItems:function(){
		var main = this.main,
			offsetY = main.getOffsets().y,container = this.container,
			top = container.getScroll().y,vh=container.getSize().y ;

        offsetY += main.getSize().y;
        offsetY -= this.options.threshold;
        return offsetY <= top + vh ? true : false;
    },
	request:function(){
		var self = this, count = 2, sync =self.sync, 
			syncOpt =this.options.syncOpt, data = syncOpt.data,
			data = data ? data : 'page='+this.step++,
			_onSuccess= syncOpt.onSuccess || function(){};

        if(this.isComplete || (sync && sync.running))return this;
		return sync = this.sync= new Request.HTML(
			Object.append(syncOpt,{
				timeout:30000,
				append:this.main,
				data:data,
				onTimeout:function(async){
					this.cancel();
					if(!count) return self.fireEvent('failure');
					count -= 1;this.send();
				},
				onSuccess:function(rs){
					_onSuccess.apply(self, arguments);
					if(self.isComplete) self.onSuccess.call(self);
				}
			})).send();
     },
	 onSuccess:function(){return this.fireEvent('success');}
});
