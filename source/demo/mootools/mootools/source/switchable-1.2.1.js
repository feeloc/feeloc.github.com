...
var Switchable = new Class({
	...
	effInit: function() {
		...
		len = panels.length;
		this.viewSize = [
		options.viewSize[0] || panels[0].getSize().x * steps, options.viewSize[1] || panels[0].getSize().y * steps];
		if (effect !== 'none') {
			switch (effect) {
			case 'scrollx':
			case 'scrolly':
...
Switchable.autoRender = function(autoClass, container) {
	...
	if (clt.length) clt.each(function(el) {
		...
		config;
		if (type && ('Tabs Switchable DropMenu Accordion DataLazyLoad Popup Countdown'.indexOf(type) > - 1)) {
			try {
...
var Countdown = new Class({
	...
	rendDom: function(time) {
		...
		var dDom = $(this.options.timeUnitCls.d) || this.element.getElement(this.options.timeUnitCls.d);
		if (dDom) dDom.set('html', this.addZero(d));
		var hDom = $(this.options.timeUnitCls.h) || this.element.getElement(this.options.timeUnitCls.h);
		if (hDom) hDom.set('html', this.addZero(h));
		var mDom = $(this.options.timeUnitCls.m) || this.element.getElement(this.options.timeUnitCls.m);
		if (mDom) mDom.set('html', this.addZero(m));
		var sDom = $(this.options.timeUnitCls.s) || this.element.getElement(this.options.timeUnitCls.s);
		if (sDom) sDom.set('html', this.addZero(s));
	},
	startRun: function(time, interval) {
		$(this.options.timeEndCls) || this.element.getElements(this.options.timeEndCls).setStyle('display', 'none');
		$(this.options.timeRunCls) || this.element.getElements(this.options.timeRunCls).setStyle('display', 'block');
		...
		if (this.options.isMsecond) {
			var num = 9;
			if(!$(this.options.timeUnitCls.i)) return;
			this.i = setInterval(function() {
				$(this.options.timeUnitCls.i) || this.element.getElement(this.options.timeUnitCls.i).set('html', num--);
				if (num < 0) num = 9;
			}.bind(this), 100);
		}
	},
	endRun: function() {
		$(this.options.timeRunCls) || this.element.getElements(this.options.timeRunCls).setStyle('display', 'none');
		$(this.options.timeEndCls) || this.element.getElements(this.options.timeEndCls).setStyle('display', 'block');
		this.fireEvent('end');
	}
});
